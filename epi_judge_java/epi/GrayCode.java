...
public class GrayCode {
    ...
    private static boolean directedGrayCode(int numBits, Set<Integer> history, List<Integer> result) {
        if (result.size() == (1 << numBits)) return differsByOneBit(result.get(0), result.get(result.size() - 1));
        for (int i = 0; i < numBits; ++i) {
            ...
            if (!history.contains(candidateNextCode)) {
                ...
                if (directedGrayCode(numBits, history, result)) return true;
    ...
    public static void grayCodeWrapper(TimedExecutor executor, int numBits) throws Exception {
        ...
        if (result.size() != expectedSize)
            throw new TestFailure("Length mismatch: expected " + String.valueOf(expectedSize) + ", got " + String.valueOf(result.size()));
        for (int i = 1; i < result.size(); i++)
            if (!differsByOneBit(result.get(i - 1), result.get(i)))
                if (result.get(i - 1).equals(result.get(i))) throw new TestFailure("Two adjacent entries are equal");
                else throw new TestFailure("Two adjacent entries differ by more than 1 bit");
        Set<Integer> uniq = new HashSet<>(result);
        if (uniq.size() != result.size())
            throw new TestFailure("Not all entries are distinct: found " + String.valueOf(result.size() - uniq.size()) + " duplicates");
    }

    public static void main(String[] args) {
        System.exit(GenericTest.runFromAnnotations(args, "GrayCode.java", new Object() {
