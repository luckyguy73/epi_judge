...
public class NumberOfTraversalsMatrix {
    ...
    private static int computeNumberOfWaysToXY(int x, int y, int[][] dp) {
        if (x == 0 && y == 0) return 1;
        if (dp[x][y] == 0) {
            int waysTop = x == 0 ? 0 : computeNumberOfWaysToXY(x - 1, y, dp);
            int waysLeft = y == 0 ? 0 : computeNumberOfWaysToXY(x, y - 1, dp);
