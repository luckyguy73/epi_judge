...
public class SearchMaze {
    ...
    private static boolean searchMazeHelper(List<List<Color>> maze, Coordinate cur, Coordinate e,
        ...
        for (Coordinate nextMove : List.of(new Coordinate(cur.x, cur.y + 1),
                new Coordinate(cur.x, cur.y - 1), new Coordinate(cur.x + 1, cur.y),
                new Coordinate(cur.x - 1, cur.y)))
            if (searchMazeHelper(maze, nextMove, e, path)) return true;
    ...
    public static boolean searchMazeWrapper(List<List<Integer>> maze, Coordinate s, Coordinate e) throws
            TestFailure {
        List<List<Color>> colored = new ArrayList<>();
        for (List<Integer> col : maze) {
            List<Color> tmp = new ArrayList<>();
            for (Integer i : col) tmp.add(i == 0 ? Color.WHITE : Color.BLACK);
            colored.add(tmp);
        }
        List<Coordinate> path = searchMaze(colored, s, e);
        if (path.isEmpty()) return s.equals(e);
