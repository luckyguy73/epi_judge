...
public class RefuelingSchedule {
    ...
    public static int findAmpleCity(List<Integer> gallons, List<Integer> distances) {
        ...
        for (int i = 1; i < numCities; ++i) {
            ...
            if (remainingGallons < min.remainingGallons) min = new CityAndRemainingGas(i, remainingGallons);
        }
        return min.city;
    }

    @EpiTest(testDataFile = "refueling_schedule.tsv")
    public static void findAmpleCityWrapper(TimedExecutor executor, List<Integer> gallons, List<Integer> distances) throws Exception {
        ...
        for (int i = 0; i < numCities; ++i) {
            ...
            if (tank < 0) throw new TestFailure(String.format("Out of gas on city %d", city));
        }
    }

    public static void main(String[] args) {
        System.exit(GenericTest.runFromAnnotations(args, "RefuelingSchedule.java", new Object() {
