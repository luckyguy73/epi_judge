...
public class LongestNondecreasingSubsequence {
    ...
    public static int longestNondecreasingSubsequenceLength(List<Integer> A) {
        ...
        for (int i = 1; i < A.size(); ++i)
            for (int j = 0; j < i; ++j)
                if (A.get(i) >= A.get(j)) maxLength[i] = Math.max(maxLength[i], maxLength[j] + 1);
        return Collections.max(List.of(maxLength));
    }

    public static void main(String[] args) {
        System.exit(GenericTest.runFromAnnotations(args, "LongestNondecreasingSubsequence.java", new Object() {
