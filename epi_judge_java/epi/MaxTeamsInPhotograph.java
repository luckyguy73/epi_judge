...
public class MaxTeamsInPhotograph {
    ...
    public static int findLargestNumberTeams(List<GraphVertex> graph) {
        int maxLevel = 0;
        for (GraphVertex g : graph) if (g.maxDistance == 0) maxLevel = Math.max(maxLevel, dfs(g));
        return maxLevel;
    }
    ...
    private static int dfs(GraphVertex curr) {
        ...
        for (GraphVertex vertex : curr.edges)
            curr.maxDistance = Math.max(curr.maxDistance, (vertex.maxDistance != 0 ? vertex.maxDistance : dfs(vertex)) + 1);
    ...
    public static int findLargestNumberTeamsWrapper(TimedExecutor executor, int k, List<Edge> edges) throws Exception {
    ...
    public static void main(String[] args) {
        System.exit(GenericTest.runFromAnnotations(args, "MaxTeamsInPhotograph.java", new Object() {
