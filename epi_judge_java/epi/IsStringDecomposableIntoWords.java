...
public class IsStringDecomposableIntoWords {
    ...
    public static void decomposeIntoDictionaryWordsWrapper(TimedExecutor executor, String domain, Set<String> dictionary, Boolean decomposable) throws Exception {
        List<String> result = executor.run(() -> decomposeIntoDictionaryWords(domain, dictionary));
        if (!decomposable) {
            if (!result.isEmpty()) throw new TestFailure("domain is not decomposable");
            return;
        }
        if (result.stream().anyMatch(s -> !dictionary.contains(s)))
            throw new TestFailure("Result uses words not in dictionary");
        if (!String.join("", result).equals(domain)) throw new TestFailure("Result is not composed into domain");
    }

    public static void main(String[] args) {
        System.exit(GenericTest.runFromAnnotations(args, "IsStringDecomposableIntoWords.java", new Object() {
