...
public class GraphClone {
    ...
    public static GraphVertex cloneGraph(GraphVertex graph) {
        ...
        while (!q.isEmpty()) {
            ...
            for (GraphVertex e : v.edges) {
                if (vertexMap.putIfAbsent(e, new GraphVertex(e.label)) == null) q.add(e);
    ...
    private static List<Integer> copyLabels(List<GraphVertex> edges) {
        ...
        for (GraphVertex e : edges) labels.add(e.label);
        return labels;
    }

    private static void checkGraph(GraphVertex node, List<GraphVertex> graph) throws TestFailure {
        if (node == null) throw new TestFailure("Graph was not copied");
        ...
        while (!q.isEmpty()) {
            ...
            if (vertex.label >= graph.size()) throw new TestFailure("Invalid vertex label");
            List<Integer> label1 = copyLabels(vertex.edges), label2 = copyLabels(graph.get(vertex.label).edges);
            Collections.sort(label1);
            Collections.sort(label2);
            if (!label1.equals(label2)) throw new TestFailure("Edges mismatch");
    ...
    public static void cloneGraphTest(int k, List<Edge> edges) throws TestFailure {
        ...
        for (int i = 0; i < k; i++) graph.add(new GraphVertex(i));
        for (Edge e : edges) {
            if (e.from < 0 || e.from >= k || e.to < 0 || e.to >= k) throw new RuntimeException("Invalid vertex index");
    ...
    public static void main(String[] args) {
        System.exit(GenericTest.runFromAnnotations(args, "GraphClone.java", new Object() {
