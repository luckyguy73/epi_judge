...
public class IsCircuitWirable {
        ...
        while (!q.isEmpty()) {
            for (GraphVertex t : q.peek().edges)
                if (t.d == -1) {
                    t.d = q.peek().d + 1;
                    q.add(t);
                } else if (t.d == q.peek().d) return false;
            q.remove();
        }
        return true;
    }
    ...
    public static boolean isAnyPlacementFeasibleWrapper(TimedExecutor executor, int k, List<Edge> edges) throws Exception {
    ...
    public static void main(String[] args) {
        System.exit(GenericTest.runFromAnnotations(args, "IsCircuitWirable.java", new Object() {
