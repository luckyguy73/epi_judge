...
public class BinomialCoefficients {
    ...
    private static int computeBinomialCoefficientsSpaceEfficient(int n, int k) {
        ...
        table.set(0, 1);
        for (int i = 1; i <= n; ++i)
            for (int j = Math.min(i, k); j >= 1; --j) table.set(j, table.get(j) + table.get(j - 1));
        return table.get(k);
    }

    public static void main(String[] args) {
        System.exit(GenericTest.runFromAnnotations(args, "BinomialCoefficients.java", new Object() {
